// Shared subagent sandbox foundation (HAR-26). Every subagent needs some
// subset of the root's sandbox recipe - repo checkout, the HAR-3 CLI
// toolchain, and the network-boundary git/gh auth brokering - but declared
// subagents inherit nothing from the root (own sandbox, see
// `node_modules/eve/docs/subagents.mdx`). This module is that recipe,
// parameterized so both `agent/sandbox.ts` and each
// `agent/subagents/<id>/sandbox.ts` compose it instead of duplicating it.
//
// `createSandboxRecipe` is the single entry point: it returns the
// `backend`/`revalidationKey`/`bootstrap`/`onSession` shape `defineSandbox`
// expects. Everything else here is exported piecemeal too, so a caller that
// needs one piece (e.g. a test, or a subagent that wants a custom
// `onSession`) can reach for it directly instead of going through the whole
// recipe.
 ...
 * it. Seeded unconditionally (even for a `gitAuth: "none"` recipe): it only
 * satisfies `gh`'s local login check, and a session with no auth-carrying
 * network policy still gets a real 401/403 from GitHub on any authenticated
 * call, so there's nothing to gate here.
 */
...
/** Options for {@link buildBootstrapCommand}. */
export interface BootstrapCommandOptions {
  /** Installs and verifies Playwright chromium for `scripts/play-web.mjs` screenshots. Default `true` (the root's historical behavior). */
  screenshotTooling?: boolean;
}

/**
 * Builds the sandbox pre-warm command: system packages, repo clone,
 * dependency install, and (optionally) the Playwright chromium
 * `scripts/play-web.mjs`'s screenshots depend on. Exported (rather than
 * inlined in a sandbox's `bootstrap`) so its content - in particular, that it
 * actually verifies chromium can launch instead of trusting the install step
 ...
 * `gh`, `pi`+ponytail) are unconditional - every subagent needs some subset of
 * them, and installing all of them regardless of which subset costs nothing
 * (the image is baked once and reused via {@link dependencyRevalidationKey}).
 * Screenshot tooling is the one part expensive enough (a Playwright chromium
 * install) to make opt-in worthwhile - only the playtester subagent needs it.
 */
export function buildBootstrapCommand(
  options: BootstrapCommandOptions = {},
): string {
  const { screenshotTooling = true } = options;
  ...
  return [
    ...
    // interactive `play dev` layout, and (when enabled) Playwright's chromium
    // backs the web play harness (scripts/play-web.mjs), so the agent can
    // drive and screenshot both renderers in-sandbox. ripgrep/fd-find/bat/eza
    // are the agent-friendly replacements for grep/find/cat/ls (faster,
    // .gitignore aware, better defaults - see HAR-3); ast-grep adds
    // structural (syntax-tree) code search on top of them for refactors and
    // call-site queries plain text search can't express. `gh` replaces raw
    // `curl` + the GitHub REST API for pull-request operations (HAR-14).
...
// Best-effort: pre-computes the orientation brief so the model reads settled
// repo state instead of rediscovering it. Never fails the session - a missing
// brief just means the model falls back to orienting by hand.
async function writeOrientationBrief(
  ...
  try {
    ...
    // rather than re-running the chromium-launch check per session. Reading
    // it is harmless even for a recipe that opted out of screenshot tooling -
    // the missing status file just reads back as unavailable.
...
/**
 * Level of GitHub network-boundary auth a sandbox's live session gets
 * (bootstrap always mints a token once, regardless of level, to clone the
 * private repo into the snapshot - this only governs the *live* session):
 *
 * - `"none"`: the live session's network policy carries no `github.com` auth
 *   header, there is no token-refresh loop, and stranded-push recovery is
 *   skipped. The session can read the checked-out repo but cannot push or
 *   call an authenticated GitHub API. For a subagent that only needs to read
 *   code (e.g. `scout`).
 * - `"read-only"`: the live session keeps an authenticated network policy,
 *   refreshed on an interval, so `gh api`/`git fetch` work - but stranded-push
 *   recovery is skipped. For a subagent that calls GitHub's API (e.g.
 *   `reviewer` posting a PR review) but never pushes code. The underlying
 *   GitHub App token is the same all-scopes token `"push-capable"` gets
 *   (Vercel Connect mints one token per app installation, not a token per
 *   requested permission subset); the distinction here is deliberate policy
 *   for what this sandbox recipe wires up, not a mechanically narrower
 *   credential.
 * - `"push-capable"`: `"read-only"` plus stranded-push recovery, for a
 *   session expected to push its own branch (the root, and `coder`).
 */
export type GitAuthLevel = "none" | "read-only" | "push-capable";

export interface SandboxRecipeOptions {
  /** See {@link GitAuthLevel}. */
  gitAuth: GitAuthLevel;
  /** Installs and verifies Playwright chromium for `scripts/play-web.mjs` screenshots. Default `false` - only `playtester` needs it. */
  screenshotTooling?: boolean;
  /** Sandbox lifetime ceiling, re-asserted on every session attach. Default {@link SANDBOX_TIMEOUT_MS} (5h, the Vercel Sandbox platform ceiling). */
  timeoutMs?: number;
}

/**
 * Builds the `backend`/`revalidationKey`/`bootstrap`/`onSession` shape a
 * sandbox module passes to `defineSandbox`. This is the composable recipe
 * HAR-26 extracts: the root's `agent/sandbox.ts` calls this with
 * `{ gitAuth: "push-capable", screenshotTooling: true }` (its historical,
 * unparameterized behavior), and a subagent's `agent/subagents/<id>/sandbox.ts`
 * expresses its narrower needs in the same call:
 *
 * ```ts
 ...
 * ```
 */
export function createSandboxRecipe(
...
> {
  ...
  const gitAuthEnabled = gitAuth !== "none";

  return {
    backend: vercel({ timeout: timeoutMs }),
    revalidationKey: dependencyRevalidationKey,
    async bootstrap({ use }) {
      // Bootstrap always authenticates once to clone the private repo into
      // the snapshot, regardless of `gitAuth` - that level only governs the
      // *live* session's network policy below.
      const { policy } = await resolveStartupNetworkPolicy();
      const sandbox = await use({ networkPolicy: policy });
      const setup = await sandbox.run({
        command: buildBootstrapCommand({ screenshotTooling }),
      });
      if (setup.exitCode !== 0)
        throw new Error(setup.stderr || "Sandbox pre-warming failed");
    },
    async onSession({ use }) {
      const { policy, authed } = gitAuthEnabled
        ? await resolveStartupNetworkPolicy()
        : { policy: OPEN_NETWORK_POLICY, authed: false };
      ...
      const sandbox = await use({ networkPolicy: policy, timeout: timeoutMs });
      ...
      // Only for a recipe expected to push; best-effort and skipped entirely
      // when unauthenticated, since it would just fail the same way the
      // agent's own push already would.
      if (authed && gitAuth === "push-capable") {
        try {
      ...
      await writeOrientationBrief(sandbox, authed);
      // If startup couldn't mint the token, retry soon so push recovers fast;
      // otherwise refresh on the normal cadence. Skipped for gitAuth "none" -
      // there's nothing to keep fresh when the live policy never carries auth.
      if (gitAuthEnabled) {
