...
The Workflow tool also supports concurrent fan-out: use `Promise.all` to dispatch multiple independent subagent calls in parallel (e.g., `reviewer` reviewing several open PRs, or `scout` running several independent probes). Gather the results and reduce them afterward. Only use this for genuinely independent, non-overlapping work items - anything with a dependency chain stays in a sequential workflow.
