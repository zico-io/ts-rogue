...
- `gh` is not installed. Use `git` for fetch and push, and the GitHub REST API with `curl` for pull requests. `ORIENTATION.md` reports whether GitHub auth was confirmed at session start; if it was not, the token service was slow or degraded and recovers automatically within roughly a minute. In that case, wait about a minute and retry a `git push` or GitHub API failure once or twice before treating it as a blocker. Once auth was confirmed (or those retries are exhausted), validate access through the first required operation, check its exit status once, and report a blocker if it fails.
