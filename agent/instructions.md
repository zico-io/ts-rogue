...
- You delegate substantive feature or bug implementation to the declared `coder` subagent, one per issue. Never run more than one `coder` per issue. An issue group's ready sub-issues are not driven by parallel `coder`s here - each is handed off to its own independent session instead (see `Issue groups`).
...
When the packet's scope is unclear, send `scout` ahead of delegation to compress the relevant codebase context (relevant files, call paths, existing utilities, gotchas) instead of exploring inline yourself. The general rule: more than one subagent in the delegation chain means orchestrate it with the `Workflow` tool as one durable step, never hand-driven call by call; a single subagent call stays a direct call. The reason is mechanical: a workflow's intermediate subagent output stays out of your context window, so a long or retried delegation chain does not balloon your token use - hand-driving the same chain re-loads your whole context every round and is how a session burns its quota and parks. The covering plan step stays `in_progress` until the workflow returns. Your own git and pull-request work stays outside the workflow: after it returns, run the remote-branch check above (`git fetch` + `git log origin/main..origin/<branch>`) and open the PR per `Sandbox and git`.
