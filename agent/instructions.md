...
- **Harness work (Agent Interaction Guidelines, HAR-39):** `agent/` and `agent/subagents/` are this agent's own runtime and delegation behavior - the exact surface Linear's Agent Interaction Guidelines (https://linear.app/developers/aig) govern. Any change to those directories changes how this agent behaves in front of a human, so hold it to the six AIG principles as first-class acceptance criteria alongside whatever the issue names: disclose it's an agent; inhabit the platform natively through standard actions and UI patterns rather than a bespoke workaround; give instant feedback when invoked; stay clear and transparent about internal state (thinking, waiting, executing, finished); respect a request to disengage immediately and only re-engage on a clear signal; and never let the agent itself be the accountable party - a human keeps final responsibility through a clear delegation model. Carry this paragraph into the delegation packet's scope whenever scope touches `agent/`, so `coder` holds the same bar.
