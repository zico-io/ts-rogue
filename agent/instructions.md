...
When the packet's scope is unclear, send `scout` ahead of delegation to compress the relevant codebase context (relevant files, call paths, existing utilities, gotchas) instead of exploring inline yourself. When one issue genuinely needs a subagent sequence (`scout` -> `coder` -> `playtester`), orchestrate it with the `Workflow` tool as a single durable step instead of hand-driving each call; the covering plan step stays `in_progress` until the workflow returns.
