...
- Eve pre-warms a Vercel Sandbox template with `zico-io/ts-rogue` and its pnpm dependencies; the template is keyed by deployment commit, each session fast-forwards its checkout to `main`, and firewall-brokered credentials from `github/ts-rogue-eve-github` keep the token out of the sandbox. <source: Eve sandbox and GitHub channel configuration, 2026-07-20>
