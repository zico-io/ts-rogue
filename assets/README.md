...
- `forgotten_plains.png` - Tiny Overworld / Forgotten Plains tileset. Feeds `grass`, `water`, `mountain`, plus `mountainSmall`/`mountainLarge` (ROG-73: same mossy-boulder family, color-matched crops of genuinely smaller/larger rock formations already on this sheet, used for a mountain tile's local-cluster-density auto-tile stand-in - `src/ui/tiles/overworldVariants.ts`). WEB-6: `grass` and `water` were re-cropped from noisy/muddy source rectangles (rock-gray bleed baked into every grass tile; ~40/64 pixels non-water in the old water crop) to the cleanest fully-opaque solid-color regions in the same sheet, found by exhaustive per-pixel purity search, not by eye. The pack also ships a `Biomes_Merging_Tiles` sheet with cross-biome edge blends, not vendored here - its dithered transitions aren't safely hand-croppable without a way to visually verify the result; see ROG-73's PR notes for the original follow-up note. WEB-6 additionally found genuine diagonal water/land auto-tile corner pieces already present on `forgotten_plains.png` itself (around the two small pond illustrations, e.g. an 8x8 water-NW/land-SE wedge at `x=8,y=40`) - correctly orienting the full 4-corner/4-edge set for `drawShoreFringe` (`src/web/render/overworldView.ts`) is deferred for the same reason: it needs a way to visually verify the result, which this pass didn't have.
- `overworld_props.png` - Tiny Overworld / All Props. Feeds `forest` (a tree) and, as of WEB-6, four small self-contained ground-clutter icons (`grassTuft`, `grassFlowerYellow`, `grassFlowerPink`, `grassPebble`) scattered sparsely over grass tiles by `grassDecoration` (`src/ui/tiles/overworldVariants.ts`) for per-tile variety.
