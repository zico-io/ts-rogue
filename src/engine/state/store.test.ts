...
import { dungeonEntryFlavor } from "../../data/dungeons";
...
describe("game store", () => {
  ...
  describe("MoveOverworld", () => {
    ...
    it("stepping onto a dungeon entrance changes scene to dungeon", () => {
      ...
      expect(after.log.at(-1)?.text).toBe(dungeonEntryFlavor("crypt"));
...
describe("Dungeon", () => {
  it("entering a dungeon entrance creates floor 1 dungeonState and switches scene", () => {
    ...
    expect(state.log.at(-1)?.text).toBe(dungeonEntryFlavor("crypt"));
  });
