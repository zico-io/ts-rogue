...
The repository also ships an Eve project agent under `agent/`. It receives work through Linear, uses Linear as its tracker, and runs repository tasks in Vercel Sandboxes pre-warmed with the repository and locked pnpm dependencies. Linear sessions show tool calls, delegation, and reasoning as plain-language native activities, and present approval prompts as native selections. Read-only Linear tools run without approval; mutations require approval on first use. Eve owns ordinary single-issue work directly and delegates only bounded independent subtasks, since child progress is not shown in the parent Linear session. GitHub credentials are brokered through Vercel Connect and do not enter the sandbox; Eve uses `git` and the GitHub REST API directly instead of probing for credentials or creating test branches.
